...
//const int RST   = 16;  // Chip Enable
//const int DAT   = 17;  // Input/Output
//const int CLK = 18;  // Serial Clock
// 创建 DS1302 对象
...
//   设置时间后, 需要注释掉下面四行设置时间的代码,并重新烧录一次. 以免掉电重新执行setup中的时间设置函数.
  ...
  //rtc.halt(false);
  //Time t(2023, 7, 3, 21, 32, 00, Time::kSaturday);  // 年月日时分秒 星期  第一次使用时设置
  //rtc.time(t);
...
void loop() {
  ...
  Serial.println(String("Temp:") + String(DHTData.readTemperature()));
  Serial.println(String("Humi:") + String(DHTData.readHumidity()));
